...
public abstract class SongPlayer {
	...
	public void setEnable10Octave(boolean enable10Octave) {
        ...
        private void start() {
                ...
                playbackTask = plugin.getServer().getScheduler().runTaskTimerAsynchronously(plugin, () -> {
                        lock.lock();
                        try {
                                ...
                                tickRemainder += 1D;
                                double delay = Math.max(0.01D, song.getDelay());

                                while (tickRemainder >= delay) {
                                        tickRemainder -= delay;

                                        if (!processPlaybackTick()) {
                                                cancelPlaybackTask();
                                                return;
                                        }
                                }
        ...
        private boolean processPlaybackTick() {
                if (!(playing || fading)) {
                        return true;
                }
