...
public class Song implements Cloneable {
	...
	private String originalAuthor;
        ...
        private final Map<Integer, List<TickNote>> tickNoteCache;
	...
	public Song(float speed, HashMap<Integer, Layer> layerHashMap,
		...
		this.originalAuthor = originalAuthor;
                ...
                this.tickNoteCache = buildTickNoteCache(layerHashMap);
        }
	 ...
	 */
        ...
        /**
         * Returns immutable list of notes scheduled for the specified tick.
         * @param tick target tick
         * @return notes to play on that tick
         */
        public List<TickNote> getNotesAtTick(int tick) {
                return tickNoteCache.getOrDefault(tick, Collections.emptyList());
        }
	 ...
	 */
        ...
        private Map<Integer, List<TickNote>> buildTickNoteCache(Map<Integer, Layer> layers) {
                Map<Integer, List<TickNote>> cache = new HashMap<>();

                for (Layer layer : layers.values()) {
                        for (Map.Entry<Integer, Note> entry : layer.getNotesAtTicks().entrySet()) {
                                cache.computeIfAbsent(entry.getKey(), key -> new ArrayList<>())
                                                .add(new TickNote(layer, entry.getValue()));
                        }
                }

                cache.replaceAll((tick, notes) -> Collections.unmodifiableList(new ArrayList<>(notes)));
